...
class TestActorsImMemory:
    uuid = []

    def test_get_actors_with_db(self, app_memory, api_ver):
        client = app_memory.test_client()
        resp = client.get(f'{api_ver}/actors')
        assert resp.status_code == http.HTTPStatus.OK

    def test_create_actor_with_db(self, app_memory, api_ver):
        client = app_memory.test_client()
        data = {
            'name': 'Test Actor',
            'birthday': '2010-04-01',
    ...
    def test_update_actor_with_db(self, app_memory, api_ver):
        client = app_memory.test_client()
    ...
    def test_delete_actor_with_db(self, app_memory, api_ver):
        client = app_memory.test_client()
        url = f'{api_ver}/actors/{self.uuid[0]}'
